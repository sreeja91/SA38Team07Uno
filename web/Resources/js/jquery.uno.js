...
;( function( $, window, undefined ) {
	...
	// the options
	$.uno.defaults = {
		// if we want to specify a selector that triggers the next() function. example: '#uno-nav-next'
		nextEl : '',
	...
	$.uno.prototype = {
		...
		_init : function( options ) {
			...
			// options
			this.options = $.extend( true, {}, $.uno.defaults, options );
		...
		_initEvents : function() {
			...
			if( this.options.nextEl !== '' ) {
				...
				$( this.options.nextEl ).on( 'click.uno', function() {
			...
			if( this.options.prevEl !== '' ) {
				...
				$( this.options.prevEl ).on( 'click.uno', function() {
			...
			this.$el.on( 'click.uno', 'li', function() {
		...
		_move2front : function( $item ) {
			...
			// if it's the one with higher z-index, just close the uno
			if( !this.closed ) {
		...
		if ( typeof options === 'string' ) {
			...
			this.each(function() {
				...
				if ( !instance ) {
					...
					logError( "cannot call methods on uno prior to initialization; " +
				...
				if ( !$.isFunction( instance[options] ) || options.charAt(0) === "_" ) {
					...
					logError( "no such method '" + options + "' for uno instance" );
					return;
			...
			this.each(function() {
				...
				if ( instance ) {
					...
					instance = $.data( this, 'uno', new $.uno( options, this ) );
